-- Create shop_order_status enum if it doesn't exist
DO $$ 
BEGIN
...
-- Add 'shop_admin' to app_role enum if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumtypid = 'public.app_role'::regtype AND enumlabel = 'shop_admin') THEN
