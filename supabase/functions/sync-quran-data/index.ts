...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (logError) {
      return new Response(JSON.stringify({ error: logError.message }), { status: 500, headers: corsHeaders })
    }
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: syncError.message }), { status: 500, headers: corsHeaders })
    }
