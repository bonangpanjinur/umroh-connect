...
export const coreApi = {
  ...
  async listCommerceWishlist() { return request<Array<Record<string, unknown>>>('/commerce/wishlist', undefined, true); },
  async addCommerceWishlist(productId: string, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>(`/commerce/wishlist/${encodeURIComponent(productId)}`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify({}) }, true); },
  async removeCommerceWishlist(productId: string, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>(`/commerce/wishlist/${encodeURIComponent(productId)}`, { method: 'DELETE', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey } }, true); },
  ...
  async getCommerceCart() { return request<Array<Record<string, unknown>>>('/commerce/cart', undefined, true); },
  async addCommerceCartItem(input: { product_id: string; quantity: number }, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>('/commerce/cart/items', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify(input) }, true); },
  async updateCommerceCartItem(itemId: string, quantity: number, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>(`/commerce/cart/items/${encodeURIComponent(itemId)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify({ quantity }) }, true); },
  async removeCommerceCartItem(itemId: string, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>(`/commerce/cart/items/${encodeURIComponent(itemId)}`, { method: 'DELETE', headers: { 'Idempotency-Key': idempotencyKey } }, true); },
  async clearCommerceCart(idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>('/commerce/cart', { method: 'DELETE', headers: { 'Idempotency-Key': idempotencyKey } }, true); },
  async getTenantContext() { return request<{ tenant_id: string; user_id: string; membership_id: string; role: string; branch_ids: string[]; channel: string; is_platform_admin: boolean }>('/tenant-context/context', undefined, true); },
  async listPrayerCategories(all = false) { return request<Array<Record<string, unknown>>>(all ? '/content/admin/prayer-categories' : '/content/prayer-categories', undefined, all); },
  async listPublicBanners() { return request<Array<Record<string, unknown>>>('/content/banners'); },
  async listJournals() { return request<Array<Record<string, unknown>>>('/journals', undefined, true); },
  async listHajiChecklists(packageType?: string) { const query = packageType ? `?package_type=${encodeURIComponent(packageType)}` : ''; return request<Array<Record<string, unknown>>>(`/haji/checklists${query}`); },
  async listMyHajiRegistrations() { return request<Array<Record<string, unknown>>>('/haji/registrations/me', undefined, true); },
  async listManagementHajiRegistrations() { return request<Array<Record<string, unknown>>>('/haji/registrations', undefined, true); },
  async createHajiRegistration(input: Record<string, unknown>, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>('/haji/registrations', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify(input) }, true); },
  async updateHajiRegistration(id: string, input: Record<string, unknown>, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>(`/haji/registrations/${encodeURIComponent(id)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify(input) }, true); },
  async getHajiStats() { return request<Record<string, number>>('/haji/stats', undefined, true); },
  async getJournal(id: string) { return request<Record<string, unknown>>(`/journals/${encodeURIComponent(id)}`, undefined, true); },
  async createJournal(input: Record<string, unknown>, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>('/journals', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify(input) }, true); },
  async updateJournal(id: string, input: Record<string, unknown>, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>(`/journals/${encodeURIComponent(id)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify(input) }, true); },
  async deleteJournal(id: string, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>(`/journals/${encodeURIComponent(id)}`, { method: 'DELETE', headers: { 'Idempotency-Key': idempotencyKey } }, true); },
  async presignJournalPhoto(journalId: string, input: { contentType: 'image/jpeg' | 'image/png' | 'image/webp'; filename: string }, idempotencyKey = crypto.randomUUID()) { return request<Record<string, any>>(`/journals/${encodeURIComponent(journalId)}/photos/presign`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify(input) }, true); },
  async commitJournalPhoto(journalId: string, input: { object_key: string; caption?: string | null; order_index?: number }, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>(`/journals/${encodeURIComponent(journalId)}/photos/commit`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify(input) }, true); },
  async uploadJournalPhotoPresigned(journalId: string, file: File, caption?: string | null, order_index?: number, idempotencyKey = crypto.randomUUID()) { const contentType = file.type as 'image/jpeg' | 'image/png' | 'image/webp'; if (!['image/jpeg','image/png','image/webp'].includes(contentType)) throw new Error('Tipe foto tidak didukung'); const presigned = await this.presignJournalPhoto(journalId, { contentType, filename: file.name }, idempotencyKey); const upload = await fetch(presigned.upload_url, { method: 'PUT', headers: { 'Content-Type': contentType }, body: file }); if (!upload.ok) throw new Error(`Upload foto gagal (${upload.status})`); return this.commitJournalPhoto(journalId, { object_key: presigned.object_key, caption, order_index }, idempotencyKey); },
  async deleteJournalPhoto(journalId: string, photoId: string, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>(`/journals/${encodeURIComponent(journalId)}/photos/${encodeURIComponent(photoId)}`, { method: 'DELETE', headers: { 'Idempotency-Key': idempotencyKey } }, true); },
  async createPrayerCategory(input: Record<string, unknown>) { return request<Record<string, unknown>>('/content/admin/prayer-categories', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify(input) }, true); },
  async updatePrayerCategory(id: string, input: Record<string, unknown>) { return request<Record<string, unknown>>(`/content/admin/prayer-categories/${encodeURIComponent(id)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify(input) }, true); },
  async deletePrayerCategory(id: string) { return request<Record<string, unknown>>(`/content/admin/prayer-categories/${encodeURIComponent(id)}`, { method: 'DELETE', headers: { 'Idempotency-Key': crypto.randomUUID() } }, true); },
  async listPrayers(all = false, categoryId?: string) { const path = all ? '/content/admin/prayers' : `/content/prayers${categoryId ? `?category_id=${encodeURIComponent(categoryId)}` : ''}`; return request<Array<Record<string, unknown>>>(path, undefined, all); },
  async getPrayer(id: string) { return request<Record<string, unknown>>(`/content/prayers/${encodeURIComponent(id)}`); },
  async createPrayer(input: Record<string, unknown>) { return request<Record<string, unknown>>('/content/admin/prayers', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify(input) }, true); },
  async updatePrayer(id: string, input: Record<string, unknown>) { return request<Record<string, unknown>>(`/content/admin/prayers/${encodeURIComponent(id)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify(input) }, true); },
  async deletePrayer(id: string) { return request<Record<string, unknown>>(`/content/admin/prayers/${encodeURIComponent(id)}`, { method: 'DELETE', headers: { 'Idempotency-Key': crypto.randomUUID() } }, true); },
  async listManasik(all = false) { return request<Array<Record<string, unknown>>>(all ? '/content/admin/manasik' : '/content/manasik', undefined, all); },
  async listManagementItinerary(departureId: string) { return request<Array<Record<string, unknown>>>(`/management/departures/${encodeURIComponent(departureId)}/itinerary`, undefined, true); },
  async upsertManagementItinerary(departureId: string, input: Record<string, unknown>, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>(`/management/departures/${encodeURIComponent(departureId)}/itinerary`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify(input) }, true); },
  async deleteManagementItinerary(departureId: string, dayId: string, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>(`/management/departures/${encodeURIComponent(departureId)}/itinerary/${encodeURIComponent(dayId)}`, { method: 'DELETE', headers: { 'Idempotency-Key': idempotencyKey } }, true); },
  async createManasik(input: Record<string, unknown>) { return request<Record<string, unknown>>('/content/admin/manasik', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify(input) }, true); },
  async updateManasik(id: string, input: Record<string, unknown>) { return request<Record<string, unknown>>(`/content/admin/manasik/${encodeURIComponent(id)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify(input) }, true); },
  async deleteManasik(id: string) { return request<Record<string, unknown>>(`/content/admin/manasik/${encodeURIComponent(id)}`, { method: 'DELETE', headers: { 'Idempotency-Key': crypto.randomUUID() } }, true); },
  async listQuranSurahs() { return request<Array<Record<string, unknown>>>('/quran/surahs'); },
  async getQuranAdminStats() { return request<{ totalAyahs: number; totalSurahs: number; surahCounts: Record<number, number> }>('/quran/admin/stats', undefined, true); },
  async listQuranSyncLogs() { return request<Array<Record<string, unknown>>>('/quran/sync-logs', undefined, true); },
  async triggerQuranSync(mode: 'full' | 'surah' = 'full', surah_number?: number) { return request<Record<string, unknown>>('/quran/sync', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify({ mode, surah_number }) }, true); },
  async listQuranAyahs(surahNumber: number) { return request<Array<Record<string, unknown>>>(`/quran/ayahs?surah=${encodeURIComponent(String(surahNumber))}&limit=300`, undefined, false); },
  async updateQuranAyah(id: string, input: { arabic_text: string; translation_id?: string | null }) { return request<Record<string, unknown>>(`/quran/admin/ayahs/${encodeURIComponent(id)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify(input) }, true); },
  ...
  async startFreeTrial(idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>('/platform/me/trial/start', { method: 'POST', headers: { 'Idempotency-Key': idempotencyKey } }, true); },
  async createPremiumPaymentIntent(planId: string, provider: 'midtrans' | 'xendit', idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>('/platform/me/premium/payment-intents', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify({ plan_id: planId, provider }) }, true); },
  async submitPremiumPaymentProof(planId: string, paymentProofDocumentId: string, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>('/platform/me/premium/payment-proofs', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify({ plan_id: planId, payment_proof_document_id: paymentProofDocumentId }) }, true); },
  ...
  async listPrivateUserDocuments(purpose?: 'haji_registration' | 'tenant_application' | 'credit_payment_proof') { return request<Record<string, unknown>[]>(`/private-documents${purpose ? `?purpose=${purpose}` : ''}`, undefined, true); },
  ...
  async listMyCancellationRequests(bookingId?: string) { const query = bookingId ? `?booking_id=${encodeURIComponent(bookingId)}` : ''; return request<Array<Record<string, unknown>>>(`/cancellations/me${query}`, undefined, true); },
  async createCancellationRequest(input: Record<string, unknown>, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>('/cancellations', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify(input) }, true); },
  async listManagementCancellationRequests() { return request<Array<Record<string, unknown>>>('/cancellations/management', undefined, true); },
  async reviewCancellationRequest(id: string, input: Record<string, unknown>, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>(`/cancellations/${encodeURIComponent(id)}/review`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify(input) }, true); },
  ...
  async markAllMyNotificationsRead() { return request<{ ok: boolean; updated?: number }>('/notifications/read-all', { method: 'POST' }, true); },
  async savePushSubscription(input: { endpoint: string; p256dh: string; auth: string }) { return request<Record<string, unknown>>('/notifications/push-subscriptions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) }, true); },
  async deletePushSubscription(endpoint: string) { return request<{ ok: boolean }>('/notifications/push-subscriptions', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ endpoint }) }, true); },
  async listTrackingGroups() { return request<Record<string, unknown>[]>('/tracking/groups', undefined, true); },
  async createTrackingGroup(name: string, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>('/tracking/groups', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify({ name }) }, true); },
  async joinTrackingGroup(code: string, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>('/tracking/groups/join', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify({ code }) }, true); },
  async listGroupLocations(groupId: string) { return request<Record<string, unknown>[]>(`/tracking/groups/${encodeURIComponent(groupId)}/locations`, undefined, true); },
  async updateGroupLocation(groupId: string, input: Record<string, unknown>) { return request<Record<string, unknown>>(`/tracking/groups/${encodeURIComponent(groupId)}/location`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) }, true); },
  async leaveTrackingGroup(groupId: string) { return request<Record<string, unknown>>(`/tracking/groups/${encodeURIComponent(groupId)}/membership`, { method: 'DELETE' }, true); },
  async listGeofences(groupId: string) { return request<Record<string, unknown>[]>(`/tracking/groups/${encodeURIComponent(groupId)}/geofences`, undefined, true); },
  async createGeofence(groupId: string, input: Record<string, unknown>, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>(`/tracking/groups/${encodeURIComponent(groupId)}/geofences`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify(input) }, true); },
  async deleteGeofence(groupId: string, geofenceId: string, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>(`/tracking/groups/${encodeURIComponent(groupId)}/geofences/${encodeURIComponent(geofenceId)}`, { method: 'DELETE', headers: { 'Idempotency-Key': idempotencyKey } }, true); },
  async listGeofenceAlerts(groupId: string) { return request<Record<string, unknown>[]>(`/tracking/groups/${encodeURIComponent(groupId)}/alerts`, undefined, true); },
  async createGeofenceAlert(groupId: string, input: Record<string, unknown>) { return request<Record<string, unknown>>(`/tracking/groups/${encodeURIComponent(groupId)}/alerts`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) }, true); },
  async acknowledgeGeofenceAlert(groupId: string, alertId: string) { return request<Record<string, unknown>>(`/tracking/groups/${encodeURIComponent(groupId)}/alerts/${encodeURIComponent(alertId)}/ack`, { method: 'PATCH' }, true); },
  async sendWhatsAppReminder(input: { phone: string; message: string; recipientName?: string; templateCode?: string }) { return request<{ success: boolean; messageId?: string; error?: string }>('/whatsapp/send', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) }, true); },
  async getNotificationPreferences() { return request<Record<string, unknown>>('/preferences/notifications', undefined, true); },
  async updateNotificationPreferences(input: Record<string, unknown>) { return request<Record<string, unknown>>('/preferences/notifications', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) }, true); },
  async getDepartureAudit(packageId: string, limit = 100) { return request<Record<string, unknown>[]>(`/departure-audit/${encodeURIComponent(packageId)}?limit=${limit}`, undefined, true); },
  async getDepartureAuditCount(packageId: string) { const r = await request<{ count: number }>(`/departure-audit/${encodeURIComponent(packageId)}/count`, undefined, true); return r.count || 0; },
  async generatePackingList(input: { departureDate: string; returnDate: string; gender: 'male' | 'female'; duration: number }) { return request<{ success: boolean; packing_list: Record<string, unknown>; weather: Record<string, unknown> | null }>('/packing-lists/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) }, true); },
  async listChatMessages(params: { travelId: string; bookingId?: string | null }) { const q = new URLSearchParams({ travel_id: params.travelId }); if (params.bookingId) q.set('booking_id', params.bookingId); return request<Record<string, unknown>[]>(`/chat/messages?${q}`, undefined, true); },
  async sendChatMessage(input: Record<string, unknown>, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>('/chat/messages', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify(input) }, true); },
  async markChatMessagesRead(input: { travel_id: string; booking_id?: string | null }) { return request<{ ok: boolean }>('/chat/messages/read', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) }, true); },
  async getChatUnreadCount(travelId: string) { return request<{ count: number }>(`/chat/unread-count?travel_id=${encodeURIComponent(travelId)}`, undefined, true); },
  async listChatConversations() { return request<Record<string, unknown>[]>('/chat/conversations', undefined, true); },
  ...
  async requestManagementMembership(travelId: string, input: { plan_type: string; amount: number; payment_proof_document_id: string }, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>(`/management/membership/${encodeURIComponent(travelId)}/requests`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify(input) }, true); },
  async createManagementMembershipPaymentIntent(travelId: string, input: { plan_type: 'pro' | 'premium'; amount: number; provider: 'midtrans' | 'xendit' }, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>(`/management/membership/${encodeURIComponent(travelId)}/payment-intents`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify(input) }, true); },
  ...
  async requestCreditPurchase(travelId: string, input: { credits: number; amount: number; proof_document_id: string; notes?: string }, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>(`/management/featured/credits/${encodeURIComponent(travelId)}/purchase`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify(input) }, true); },
  async createCreditPaymentIntent(travelId: string, input: { credits: number; amount: number; provider: 'midtrans' | 'xendit' }, idempotencyKey = crypto.randomUUID()) { return request<Record<string, unknown>>(`/management/featured/credits/${encodeURIComponent(travelId)}/payment-intents`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey }, body: JSON.stringify(input) }, true); },
  ...
  async listPlatformPages(params?: { q?: string }) { const q = params?.q ? `?q=${encodeURIComponent(params.q)}` : ''; return request<Record<string, unknown>[]>(`/platform/admin/pages${q}`, undefined, true); },
  async getPublicPage(slug: string) { return request<Record<string, unknown>>(`/content/pages/${encodeURIComponent(slug)}`); },
  async createPlatformPage(input: Record<string, unknown>) { return request<Record<string, unknown>>('/platform/admin/pages', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify(input) }, true); },
  async updatePlatformPage(id: string, input: Record<string, unknown>) { return request<Record<string, unknown>>(`/platform/admin/pages/${encodeURIComponent(id)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify(input) }, true); },
  async deletePlatformPage(id: string) { return request<Record<string, unknown>>(`/platform/admin/pages/${encodeURIComponent(id)}`, { method: 'DELETE', headers: { 'Idempotency-Key': crypto.randomUUID() }, }, true); },
  async togglePlatformPage(id: string, isActive: boolean) { return request<Record<string, unknown>>(`/platform/admin/pages/${encodeURIComponent(id)}/active`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify({ is_active: isActive }) }, true); },
