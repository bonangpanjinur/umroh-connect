...
export const coreApi = {
  ...
  async listCommerceOrders() { return request<Array<Record<string, unknown>>>('/commerce/orders', undefined, true); },
  ...
  async listCommerceProducts(params?: { sellerId?: string; q?: string; page?: number; limit?: number }) { const search = new URLSearchParams(); if (params?.sellerId) search.set('seller_id', params.sellerId); if (params?.q) search.set('q', params.q); if (params?.page) search.set('page', String(params.page)); if (params?.limit) search.set('limit', String(params.limit)); return request<Array<Record<string, unknown>>>(`/commerce/products${search.toString() ? `?${search}` : ''}`); },
