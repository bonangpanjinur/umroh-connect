...
export const coreApi = {
  ...
  async uploadPublicAsset(input: { data: string; contentType: string; filename: string; bucket?: string }) { return request<{ url: string; publicUrl: string }>('/uploads', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) }); },
  async deletePublicAsset(path: string) { return request<{ success: boolean }>('/uploads', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ path }) }); },
