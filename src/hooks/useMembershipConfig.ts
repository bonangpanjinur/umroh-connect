...
export const useMembershipConfig=()=>useQuery({queryKey:key,queryFn:async()=>{const plans=await coreApi.listSubscriptionPlans() as CorePlan[];const mapped=plans.filter(p=>p.code!=='free' || plans.length<=1).map(mapPlan);return mapped.length?mapped:MEMBERSHIP_PLANS,staleTime:300000});
