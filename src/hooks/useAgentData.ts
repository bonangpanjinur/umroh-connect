...
export const useAgentTravel = () => {
  ...
  return useQuery({ queryKey: ['agent-travel', profile?.id], queryFn: async () => profile?.id ? await coreApi.getManagementTravel() as Travel : null, enabled: !!profile?.id });
};

export const useCreateTravel = () => {
  const queryClient = useQueryClient();
  return useMutation({ mutationFn: async (travelData: Partial<Travel>) => coreApi.updateManagementTravel(asRecord(travelData)) as Promise<Travel>, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['agent-travel'] }); toast({ title: 'Profil travel berhasil disimpan!' }); }, onError: (error: any) => toast({ title: 'Gagal menyimpan profil travel', description: error.message, variant: 'destructive' }) });
};
...
export const useCreatePackage = () => {
  const queryClient = useQueryClient();
  return useMutation({ mutationFn: async (data: Partial<Package>) => coreApi.createManagementPackage(asRecord(data)) as Promise<Package>, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['agent-packages'] }); queryClient.invalidateQueries({ queryKey: ['packages'] }); toast({ title: 'Paket berhasil dibuat!' }); }, onError: (error: any) => toast({ title: 'Gagal membuat paket', description: error.message, variant: 'destructive' }) });
};

export const useUpdatePackage = () => {
  const queryClient = useQueryClient();
  return useMutation({ mutationFn: async ({ id, ...data }: Partial<Package> & { id: string }) => coreApi.updateManagementPackage(id, asRecord(data)) as Promise<Package>, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['agent-packages'] }); queryClient.invalidateQueries({ queryKey: ['packages'] }); toast({ title: 'Paket berhasil diupdate!' }); }, onError: (error: any) => toast({ title: 'Gagal update paket', description: error.message, variant: 'destructive' }) });
};
...
export const useCreateDeparture = () => {
  const queryClient = useQueryClient();
  return useMutation({ mutationFn: async ({ packageId, ...data }: Partial<Departure> & { packageId: string }) => coreApi.createManagementDeparture(packageId, asRecord(data)) as Promise<Departure>, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['package-departures'] }); queryClient.invalidateQueries({ queryKey: ['packages'] }); toast({ title: 'Jadwal keberangkatan berhasil ditambah!' }); }, onError: (error: any) => toast({ title: 'Gagal menambah jadwal', description: error.message, variant: 'destructive' }) });
};

export const useUpdateDeparture = () => {
  const queryClient = useQueryClient();
  return useMutation({ mutationFn: async ({ id, ...data }: Partial<Departure> & { id: string }) => coreApi.updateManagementDeparture(id, asRecord(data)) as Promise<Departure>, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['package-departures'] }); queryClient.invalidateQueries({ queryKey: ['packages'] }); toast({ title: 'Jadwal berhasil diupdate!' }); }, onError: (error: any) => toast({ title: 'Gagal update jadwal', description: error.message, variant: 'destructive' }) });
};
