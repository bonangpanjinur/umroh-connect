...
export const usePages = () => useQuery({ queryKey: pagesKey, queryFn: () => coreApi.listPlatformPages() as unknown as Promise<Page[]> });
export const usePage = (slug: string) => useQuery({ queryKey: pageKey(slug), queryFn: () => coreApi.getPublicPage(slug) as unknown as Promise<Page>, enabled: Boolean(slug) });
