...
export const useShopChat = (sellerId: string | null, orderId?: string | null) => {
  ...
  const sendMessage = useMutation({ mutationFn: async ({ message, senderRole: _senderRole, attachmentUrl, attachmentType }: { message: string; senderRole: 'buyer' | 'seller'; attachmentUrl?: string; attachmentType?: string }) => { if (!user || !orderId) throw new Error('Order chat membutuhkan autentikasi dan orderId.'); return coreApi.sendCommerceOrderMessage(orderId, { message, attachment_url: attachmentUrl || null, attachment_type: attachmentType || null }); }, onSuccess: () => { queryClient.invalidateQueries({ queryKey }); }, onError: () => toast.error('Gagal mengirim pesan') });
